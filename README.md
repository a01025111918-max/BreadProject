...
기능 구현-->
헤더: 시즌, 브랜드, 메뉴, 이벤트, 상점, 로그인/로그아웃(로그인 했을 경우에는 마이페이지로 이동할 수 있도록 프로필 사진 설정)
배너: 데이터베이스에 빵의 대한 데이터를 저장하고 불러와서 구현
상품카드: 상품 카드란을 만들고 클릭하면 상세페이지로 이동
